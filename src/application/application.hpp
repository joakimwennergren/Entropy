 ...
 *
 * This function is called whenever the framebuffer of the specified window is
 * resized. It updates the application's resolution and sets a flag indicating
 * that resizing is required.
 *
 ...
 *
 * This function is triggered whenever a mouse button is pressed or released
 * within the application window. It updates the application's internal mouse
 * state based on which button was interacted with and the type of action
 * performed.
 *
 ...
 *
 * @param window A pointer to the GLFW window that received the event.
 * @param iconified A flag indicating whether the window was iconified (1) or
 * restored (0).
 */
void OnWindowIconify(GLFWwindow *window, int iconified);

/**
 * Callback function for handling changes in the content scale of a specified
 * window. This function is called when the content scale of the window changes,
 * typically due to a change in the display scaling settings for high-DPI
 * displays.
 *
 * @param window Pointer to the GLFW window that received the event.
...
struct Application {
 public:
   ...
   *
   * This constructor initializes the GLFW library, creates a GLFW window, and
   * sets various callbacks for handling window events. It also initializes
   * random seeding, sets up a timer, and registers several services, including
   * Vulkan services, ECS, 2D physics, scripting, camera management, and the
   * Vulkan renderer. Additionally, it creates a window surface and builds the
   * swapchain and framebuffers required for the Vulkan renderer.
   *
   * Upon failure to initialize GLFW or create the window, the constructor logs
   * a critical error message and exits the application.
   */
  Application();
   ...
   * the application. It destroys the GLFW window and terminates the GLFW
   * library.
   */
  virtual ~Application();
  ...
  /**
   * Pure virtual function that must be implemented to handle rendering
   * operations.
   *
   * This function is called every frame and is intended to encompass all
   * necessary drawing and rendering logic, using the provided delta-time to
   * ensure frame-rate independent updates.
   *
   * @param screen_width
   * @param screen_height
   * @param deltaTime The time difference between the current frame and the
   * previous frame in seconds.
   */
  virtual void OnRender(int screen_width, int screen_height,
                        float deltaTime) = 0;
   ...
   * - Continuously updates the timer and calculates the delta time for frame
   * updates.
   * - Manages the rendering process and script callbacks for rendering and
   * input.
   * - Handles window events and maintains the application state until the
   * window should close.
   */
  void Run();

  std::shared_ptr<Graphics::Vulkan::Renderers::VulkanRenderer>
  GetVulkanRenderer() {
    return _renderer;
  }
   ...
   *
   * This value determines how much the content is scaled in the vertical
   * direction. It is typically updated by the window content scale callback to
   * reflect changes in the window's scaling settings.
   */
  float yscale = 1.0f;
   ...
   *
   * This variable holds the current width of the screen in pixels. It is
   * primarily used for configuring rendering settings and handling window
   * resize events.
   */
  int screen_width{};
  /**
   ...
   *
   * This variable is used within the Application class to keep track of the
   * current screen height, which can be referenced in rendering and layout
   * calculations.
   */
  int screen_height{};
   ...
   *
   * This boolean variable is set to true when the application window is
   * minimized and set to false when it is restored. It helps to manage the
   * application's state and event handling related to window size changes.
   */
 ...
 private:
   ...
   * It is used to manage the timer's lifecycle and ensure that only one
   * instance of the timer exists throughout its scope. The Timer object is
   * responsible for handling timed events within the application.
   */
   ...
   *
   * This variable holds the handle to a Vulkan surface, which represents an
   * abstraction for a platform-specific surface where rendering can occur. It
   * is typically used for presenting rendered images to a display. The initial
   * value is set to VK_NULL_HANDLE, indicating that it is not yet initialized.
   */
  VkSurfaceKHR _surface = VK_NULL_HANDLE;
  /**
   * Stores the timestamp of the last game tick.
   *
   * This variable keeps track of the time at which the last game tick was
   * processed. It is used to calculate the time elapsed between ticks,
   * facilitating updates and synchronization within the game loop.
   */
  float _lastTick{};
  /**
   * Variable that represents the time difference between the current frame and
   * the previous frame.
   *
   * This value is typically used to achieve frame rate independent movement and
   * calculations in real-time applications, such as games or simulations.
   */
  float _deltaTime{};
};
}  // namespace Entropy::EntryPoints
