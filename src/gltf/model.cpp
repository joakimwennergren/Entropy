...
Mesh::Mesh(std::shared_ptr<ServiceLocator> serviceLocator, glm::mat4 matrix)
{
    ...
    tempBuffer = new Entropy::Graphics::Buffers::UniformBuffer(serviceLocator, sizeof(uniformBlock));
    uniformBuffer.memory = tempBuffer->GetBufferMemory();
    uniformBuffer.mapped = tempBuffer->GetMappedMemory();
    uniformBuffer.descriptor = {tempBuffer->GetVulkanBuffer(), 0, sizeof(uniformBlock)};
};
...
Model::Model(std::shared_ptr<ServiceLocator> serviceLocator)
{
    ...
    noTexture = new Texture(_serviceLocator);
    noTexture->CreateTextureImage(Entropy::Filesystem::GetTexturesDir() + "/checkered.png");
...
void Model::setupNodeDescriptorSet(Node *node, VkDescriptorSetLayout layout)
{
    ...
    if (node->mesh)
    {
        ...
        if (vkAllocateDescriptorSets(logicalDevice->Get(), &descriptorSetAllocInfo, &node->mesh->uniformBuffer.descriptorSet) != VK_SUCCESS)
        {
    ...
    if (node->mesh)
    {
        ...
        for (Primitive *primitive : node->mesh->primitives)
        {
            if (primitive->material.alphaMode == alphaMode)
            {
                ...
                if (isCubeMap)
                {
                    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->GetPipelineLayout(), 1, 1, &_cubeMapDS, 0, nullptr);
                }
                else
                {
...
void Model::SetupCubeMap(std::shared_ptr<Texture> texture)
{
    ...
    if (vkAllocateDescriptorSets(logicalDevice->Get(), &allocInfo, &_cubeMapDS) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to allocate descriptor sets!");
    }
