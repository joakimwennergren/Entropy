...
struct TextureFactory {
  ...
  TextureFactory(VulkanBackend backend, QueueSync qs, Allocator a): _vkBackend{backend}, _queueSync{qs}, _allocator{a} {
  }

  DepthBufferTexture CreateDepthBufferTexture(unsigned int width, unsigned int height)
  {
    return DepthBufferTexture(_vkBackend, _queueSync, _allocator, width, height);
  }

  SwapChainTexture CreateSwapChainTexture(unsigned int width, unsigned int height)
  {
