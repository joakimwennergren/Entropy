...
namespace Entropy::Physics {
  ...
  public:
    explicit BulletDebugDrawer(flecs::world *world) { _world = world; }
    ...
    void drawLine(const btVector3 &from, const btVector3 &to,
      ...
      // gizmo.set<Entropy::Components::LineGizmo>({std::make_shared<Line>(_serviceLocator,
      // f, t, c)}); gizmo.set<Entropy::Components::Renderable>({id++, 1, true});
  ...
  class Physics3D {
  public:
    explicit Physics3D(flecs::world *world) {
    ...
    void pickObject(int width, int height, int mouseX, int mouseY,
                    const glm::mat4 &projectionMatrix,
                    const glm::mat4 &viewMatrix) {
      // auto camera = _serviceLocator->GetService<Cam>();

      // // Convert screen coordinates to a 3D ray
      // glm::vec3 rayTo = getRayTo(width, height, mouseX, mouseY,
      ...
      // rayCallback(btVector3(rayFrom.x, rayFrom.y, rayFrom.z),
      // btVector3(rayEndpoint.x, rayEndpoint.y, rayEndpoint.z));
      // _dynamicsWorld->rayTest(btVector3(rayFrom.x, rayFrom.y, rayFrom.z),
      // btVector3(rayEndpoint.x, rayEndpoint.y, rayEndpoint.z), rayCallback);

      // //_dynamicsWorld->debugDrawWorld();
      // //_dynamicsWorld->getDebugDrawer()->drawLine(btVector3{rayFrom.x,
      // rayFrom.y, rayFrom.z}, btVector3{rayEndpoint.x, rayEndpoint.y,
      // rayEndpoint.z}, btVector4(0, 1, 0, 1));
      ...
      //     // Get the hit object
      //     btRigidBody *hitObject = (btRigidBody
      //     *)btRigidBody::upcast(rayCallback.m_collisionObject); if (hitObject)
      //     {
  ...
  private:
    glm::vec3 getRayTo(int width, int height, int mouseX, int mouseY,
                       const glm::mat4 &projectionMatrix,
                       const glm::mat4 &viewMatrix) {
