...
private:
  std::vector<VkDescriptorSetLayout> Setup() {
    ...
    std::vector<VkDescriptorSetLayoutBinding> skinningBinding = {
        skinningLayout};
    auto descriptorSetLayout2 =
        std::make_shared<DescriptorsetLayout>(_logicalDevice, skinningBinding);
    ...
    descriptorSets.push_back(std::make_shared<Descriptorset>(
        _logicalDevice, _descriptorPool, descriptorSetLayout2));
