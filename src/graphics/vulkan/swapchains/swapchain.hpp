...
struct Swapchain {
  ...
  Swapchain(VulkanBackend vbe) : _backend{vbe} {
    ...
    swapChainExtent = VkExtent2D{static_cast<uint32_t>(320 * 10),
  ...
  void Build(VkSurfaceKHR surface, VkExtent2D frame) {
    ...
    spdlog::error("Buiding swapchain!!");

    swapChainImageFormat = VK_FORMAT_B8G8R8A8_UNORM;
    swapChainExtent =
        VkExtent2D{static_cast<uint32_t>(320), static_cast<uint32_t>(480)};
    SwapChainSupportDetails swapChainSupport =
        QuerySwapChainSupport(_backend.physicalDevice.Get(), surface);
    ...
    QueueFamilyIndices indices =
        QueueFamily::FindQueueFamilies(_backend.physicalDevice.Get(), surface);
    ...
    if (vkCreateSwapchainKHR(_backend.logicalDevice.Get(), &createInfo, nullptr,
    ...
    vkGetSwapchainImagesKHR(_backend.logicalDevice.Get(), _swapChain,
                            &imageCount, nullptr);
    ...
    spdlog::error("HEJSAN = {}", _swapChainImageViews.size());

    for (uint32_t i = 0; i < _swapChainImages.size(); i++) {
      auto imageView =
          ImageView(_backend, _swapChainImages[i], swapChainImageFormat);
