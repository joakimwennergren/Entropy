...
{
    _camera->setPerspective(60.0f, (float)width / (float)height, 0.1f,
                           100000.0f);
    ...
    if (_queuSync.commandBuffers.size() > 0) {
      ...
      nullptr); _queuSync.commandBuffers.clear();
    }
    ...
    memcpy((char *)_dynamicUBO->GetMappedMemory() + offset,
    &ubodyn,
           sizeof(UboDataDynamic));

    VkRect2D scissor{};
    scissor.offset = {0, 0};
    scissor.extent = {(unsigned int)width,
                      (unsigned int)height};
    ...
    vkCmdSetViewport(_commandBuffers[_currentFrame].GetCommandBuffer(), 0,
    1,&viewport);
    ...
    // vkCmdBindVertexBuffers(_commandBuffers[_currentFrame]->GetCommandBuffer(),
    //                        0, 1, vertexBuffers, offsets);
    // vkCmdBindIndexBuffer(_commandBuffers[_currentFrame]->GetCommandBuffer(),
    //                      _model->indexBuffer->GetVulkanBuffer(), 0,
    //  VK_INDEX_TYPE_UINT16);
    // vkCmdDrawIndexed(_commandBuffers[_currentFrame]->GetCommandBuffer(),
    //                  _model->indices_temp.size(), 1, 0, 0, 0);
    ...
    if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED &&
     ...
     vkCmdPipelineBarrier(_commandBuffers[_currentFrame].GetCommandBuffer(),
                          sourceStage, destinationStage, 0, 0, nullptr, 0,
                          nullptr, 1, &barrier);
