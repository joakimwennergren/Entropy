...
CommandBuffer::CommandBuffer(std::shared_ptr<ServiceLocator> serviceLocator, VkCommandBufferLevel level)
{
    ...
    _pool = std::make_unique<CommandPool>(_logicalDevice, physicalDevice, surface);
    ...
    if (vkAllocateCommandBuffers(_logicalDevice->Get(), &allocInfo, &_commandBuffer) != VK_SUCCESS)
    {
...
void CommandBuffer::RecordOnce()
{
    ...
    {
        spdlog::warn("[CommandBuffer] Error while start recording one-time command buffer.");
    }
}
