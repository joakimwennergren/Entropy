...
LogicalDevice::LogicalDevice(std::shared_ptr<PhysicalDevice> physicalDevice, std::shared_ptr<WindowSurface> surface)
{
    ...
    if (vkCreateDevice(physicalDevice->Get(), &createInfo, nullptr, &_logicalDevice) != VK_SUCCESS)
    {
        exit(EXIT_FAILURE);
    }
