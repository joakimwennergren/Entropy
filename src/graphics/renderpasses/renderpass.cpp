...
void RenderPass::Begin(std::shared_ptr<CommandBuffer> commandBuffer, uint32_t imageIndex) const
{
 ...
 * @return (void)
 */
void RenderPass::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage)
{
    ...
    vmaCreateImage(allocator->Get(), &imageInfo, &allocCreateInfo, &_depthImage, &_allocation, nullptr);
}

/**
 * @brief Find memory type index
 * @param typeFilter uint32_t type filter
 * @param properties VkMemoryPropertyFlags property flags
...
void RenderPass::RecreateDepthBuffer()
{
    ...
    CreateImage(_swapChain->swapChainExtent.width, _swapChain->swapChainExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT);
    depthImageView = ImageView(_logicalDevice->Get(), _depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT).Get();
}
