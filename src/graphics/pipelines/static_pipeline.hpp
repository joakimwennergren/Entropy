...
namespace Entropy
{
    namespace Graphics
    {
        namespace Pipelines
        {
            ...
            {
            public:
                StaticPipeline(std::shared_ptr<RenderPass> renderPass, std::shared_ptr<ServiceLocator> serviceLocator, VkPolygonMode polygonMode) : Pipeline(renderPass, serviceLocator, polygonMode)
                {
                    ...
                    Build("StaticPipeline", "static_vert.spv", "static_frag.spv", dsLayouts, VK_TRUE, depthBounds, pipelineLayoutInfo, polygonMode);
                }

                StaticPipeline(std::shared_ptr<RenderPass> renderPass, std::shared_ptr<ServiceLocator> serviceLocator, std::vector<char> vert_shader, std::vector<char> frag_shader, VkPolygonMode polygonMode) : Pipeline(renderPass, serviceLocator, polygonMode)
                {
                    auto dsLayouts = Setup();
                    VkPipelineLayoutCreateInfo pipelineLayoutInfo{};
                    pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
                    ...
                    pipelineLayoutInfo.pushConstantRangeCount = 0;
                    glm::vec2 depthBounds = {0.0, 1.0};
                    Build("StaticPipeline", vert_shader, frag_shader, dsLayouts, VK_TRUE, depthBounds, pipelineLayoutInfo, polygonMode);
                }
