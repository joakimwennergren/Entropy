        ...
        namespace Pipelines
        {
            ...
            public:
                LinePipeline(std::shared_ptr<RenderPass> renderPass, std::shared_ptr<ServiceLocator> serviceLocator, VkPolygonMode polygonMode) : Pipeline(renderPass, serviceLocator, polygonMode)
                {
                    ...
                    Build("LinePipeline", "gizmo_vert.spv", "gizmo_frag.spv", dsLayouts, VK_TRUE, pipelineLayoutInfo, polygonMode);
                }
                ...
                LinePipeline(std::shared_ptr<RenderPass> renderPass, std::shared_ptr<ServiceLocator> serviceLocator, std::vector<char> vert_shader, std::vector<char> frag_shader, VkPolygonMode polygonMode) : Pipeline(renderPass, serviceLocator, polygonMode)
                {
                    ...
                    pipelineLayoutInfo.pPushConstantRanges = nullptr; //&push_constant;
                    pipelineLayoutInfo.pushConstantRangeCount = 0;
                    Build("LinePipeline", vert_shader, frag_shader, dsLayouts, VK_TRUE, pipelineLayoutInfo, VK_POLYGON_MODE_LINE);
                }
