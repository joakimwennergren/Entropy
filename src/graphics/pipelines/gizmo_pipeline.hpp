...
namespace Entropy
{
    namespace Graphics
    {
        namespace Pipelines
        {
            ...
            {
            public:
                GizmoPipeline(std::shared_ptr<RenderPass> renderPass, std::shared_ptr<ServiceLocator> serviceLocator) : Pipeline(renderPass, serviceLocator)
                {
                    ...
                    Build("GizmoPipeline", "gizmo_vert.spv", "gizmo_frag.spv", dsLayouts, VK_TRUE, pipelineLayoutInfo);
                }

                GizmoPipeline(std::shared_ptr<RenderPass> renderPass, std::shared_ptr<ServiceLocator> serviceLocator, std::vector<char> vert_shader, std::vector<char> frag_shader) : Pipeline(renderPass, serviceLocator)
                {
                    auto dsLayouts = Setup();
                    VkPipelineLayoutCreateInfo pipelineLayoutInfo{};
                    pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
                    ...
                    pipelineLayoutInfo.pPushConstantRanges = nullptr; //&push_constant;
                    pipelineLayoutInfo.pushConstantRangeCount = 0;
                    Build("GizmoPipeline", vert_shader, frag_shader, dsLayouts, VK_TRUE, pipelineLayoutInfo);
                }
