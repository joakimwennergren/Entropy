...
namespace Entropy
{
    namespace Graphics
    {
        namespace Pipelines
        {
            ...
            {
            public:
                GizmoPipeline(std::shared_ptr<RenderPass> renderPass, std::shared_ptr<ServiceLocator> serviceLocator, VkPolygonMode polygonMode) : Pipeline(renderPass, serviceLocator, polygonMode)
                {
                    ...
                    Build("GizmoPipeline", "gizmo_vert.spv", "gizmo_frag.spv", dsLayouts, VK_TRUE, depthBounds, pipelineLayoutInfo, polygonMode);
                }

                GizmoPipeline(std::shared_ptr<RenderPass> renderPass, std::shared_ptr<ServiceLocator> serviceLocator, std::vector<char> vert_shader, std::vector<char> frag_shader, VkPolygonMode polygonMode) : Pipeline(renderPass, serviceLocator, polygonMode)
                {
                    auto dsLayouts = Setup();
                    VkPipelineLayoutCreateInfo pipelineLayoutInfo{};
                    pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
                    ...
                    Build("GizmoPipeline", vert_shader, frag_shader, dsLayouts, VK_TRUE, depthBounds, pipelineLayoutInfo, VK_POLYGON_MODE_LINE);
                }
