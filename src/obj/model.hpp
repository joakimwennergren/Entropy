    ...
    namespace OBJ
    {
        ...
        public:
            ...
            void loadFromFile(std::string file, std::string tex)
            {
                ...
                vertexBuffer = std::make_unique<VertexBuffer>(_serviceLocator, vertices);

                texture = new Texture(_serviceLocator);
                texture->CreateTextureImage(tex);

                VkPhysicalDeviceProperties properties{};
                vkGetPhysicalDeviceProperties(_serviceLocator->GetService<PhysicalDevice>()->Get(), &properties);
                ...
                if (vkCreateSampler(_serviceLocator->GetService<LogicalDevice>()->Get(), &samplerInfo, nullptr, &_textureSampler) != VK_SUCCESS)
                {
                ...
                if (vkCreateDescriptorSetLayout(_serviceLocator->GetService<LogicalDevice>()->Get(), &layoutInfo, nullptr, &_descriptorSetLayout) != VK_SUCCESS)
                {
                ...
                allocInfo.descriptorPool = _serviceLocator->GetService<DescriptorPool>()->Get();
                allocInfo.descriptorSetCount = 1; // MAX_CONCURRENT_FRAMES_IN_FLIGHT;
                allocInfo.pSetLayouts = layouts.data();

                if (vkAllocateDescriptorSets(_serviceLocator->GetService<LogicalDevice>()->Get(), &allocInfo, &ds) != VK_SUCCESS)
                {
                ...
                vkUpdateDescriptorSets(_serviceLocator->GetService<LogicalDevice>()->Get(), static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
            }
